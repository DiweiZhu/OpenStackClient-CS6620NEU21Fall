...
* One of our high-level goals heading into this project was that we would need to identify gaps present in the OpenStackSDK compared to the project-specific client. As we have seen above, we have already identified and proposed a few changes to the SDK. Some of the commands that we worked on over the course of this project depends on these changes. However, for these changes to be available in the SDK, we would need to wait for the next publically available release of OpenStackSDK, which is expected to be sometime next year. As a result, some of the commands that we worked on are blocked until the new release. Fortunately, our commands have already integrated the expected new changes in the SDK. As soon as the next release is available, our commands would be ready to be merged in OpenStackClient.
